...
function validateTransaction(parsed, accounts) {
  ...
  if (parsed == null || parsed.error) {
    return { valid: false, status_code: parsed && parsed.error ? parsed.error.status_code : 'SY03', status_reason: parsed && parsed.error ? parsed.error.status_reason : 'Malformed instruction' };
  }
  ...
  if (!utils.isPositiveIntegerString(String(amountStr))) {
    return { valid:false, status_code: 'AM01', status_reason: 'Amount must be a positive integer' };
  }
  ...
  if (!utils.isValidAccountId(debitId)) {
    return { valid:false, status_code: 'AC04', status_reason: `Invalid account ID format for debit account: ${debitId}` };
  }
  if (!utils.isValidAccountId(creditId)) {
    return { valid:false, status_code: 'AC04', status_reason: `Invalid account ID format for credit account: ${creditId}` };
  }

  // accounts exist
  const debitFound = findAccountById(accounts, debitId);
  const creditFound = findAccountById(accounts, creditId);
  if (!debitFound) return { valid:false, status_code: 'AC03', status_reason: `Account not found: ${debitId}` };
  if (!creditFound) return { valid:false, status_code: 'AC03', status_reason: `Account not found: ${creditId}` };
  ...
  if (accDebitCurrency !== currency) {
    return { valid:false, status_code: 'CU01', status_reason: `Instruction currency ${currency} does not match account currency ${accDebitCurrency}` };
  }

  // debit and credit must differ
  if (debitId === creditId) {
    return { valid:false, status_code: 'AC02', status_reason: 'Debit and credit accounts cannot be the same' };
  }
  ...
  if (debitBalance < amount) {
    return { valid:false, status_code: 'AC01', status_reason: `Insufficient funds in debit account: has ${debitBalance} ${accDebitCurrency}, needs ${amount} ${accDebitCurrency}` };
  }
