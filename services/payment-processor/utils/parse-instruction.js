...
function parseInstruction(instruction) {
  ...
  if (s.length === 0)
    return { error: { status_code: 'SY03', status_reason: 'Malformed instruction: empty' } };
  ...
  if (first === 'DEBIT') {
    ...
    if (parts.length < 11)
      return { error: { status_code: 'SY03', status_reason: 'Malformed DEBIT instruction' } };
    ...
    if (
      ...
      return {
        error: { status_code: 'SY02', status_reason: 'Invalid keyword order for DEBIT format' },
      };
    }
    amountStr = parts[1];
    ...
    if (parts.length <= 10)
      return { error: { status_code: 'SY03', status_reason: 'Missing destination account' } };
    creditAccount = parts[10];
    // optional ON
    if (parts.length > 11 && parts[11].toUpperCase() === 'ON') {
      if (parts.length <= 12)
        return { error: { status_code: 'DT01', status_reason: 'ON clause missing date' } };
      executeBy = parts[12];
    }
  } else if (first === 'CREDIT') {
    type = 'CREDIT';
    if (parts.length < 11)
      return { error: { status_code: 'SY03', status_reason: 'Malformed CREDIT instruction' } };
    if (
      ...
      return {
        error: { status_code: 'SY02', status_reason: 'Invalid keyword order for CREDIT format' },
      };
    }
    amountStr = parts[1];
    currency = parts[2].toUpperCase();
    ...
    if (parts.length <= 10)
      return { error: { status_code: 'SY03', status_reason: 'Missing source account' } };
    ...
    return {
      error: {
        status_code: 'SY01',
        status_reason: 'Missing required keyword DEBIT or CREDIT at start',
      },
    };
  }
